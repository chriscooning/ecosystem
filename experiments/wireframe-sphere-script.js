...
const buildWireframeSphereData = () => {
  const latSegments = 9;
  const lonSegments = 18;
  const points = [];
  const edges = [];

  const indexGrid = Array.from({ length: latSegments + 1 }, () =>
    Array.from({ length: lonSegments }, () => -1)
  );
  ...
  for (let i = 0; i <= latSegments; i += 1) {
    for (let j = 0; j < lonSegments; j += 1) {
      ...
      const nextLon = indexGrid[i][(j + 1) % lonSegments];
      edges.push([current, nextLon]);
      if (i < latSegments) {
        const nextLat = indexGrid[i + 1][j];
...
const shapeData = buildWireframeSphereData();
