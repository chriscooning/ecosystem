...
const buildInfinityLoopData = () => {
  const centerSamples = 80;
  ...
  for (let i = 0; i < centerSamples; i += 1) {
    const t = (i / centerSamples) * Math.PI * 2;
    const denom = 1 + Math.sin(t) * Math.sin(t);
    const x = Math.cos(t) / denom;
    const y = (Math.sin(t) * Math.cos(t)) / denom;
    centerline.push({ x, y, z: 0 });
  }

  let prevNormal = { x: 0, y: 1, z: 0 };
  const ringIndex = [];

  centerline.forEach((center, i) => {
    ...
    const binormal = normalizePoint({
      x: tangent.y * prevNormal.z - tangent.z * prevNormal.y,
      y: tangent.z * prevNormal.x - tangent.x * prevNormal.z,
      z: tangent.x * prevNormal.y - tangent.y * prevNormal.x,
    });
    ...
    for (let j = 0; j < ringSegments; j += 1) {
      ...
      const point = {
        x: center.x + (normal.x * cos + binormal.x * sin) * tubeRadius,
        y: center.y + (normal.y * cos + binormal.y * sin) * tubeRadius,
        z: center.z + (normal.z * cos + binormal.z * sin) * tubeRadius,
      };
  ...
  ringIndex.forEach((ring, i) => {
    const nextRing = ringIndex[(i + 1) % ringIndex.length];
    for (let j = 0; j < ring.length; j += 1) {
      const next = (j + 1) % ring.length;
      edges.push([ring[j], ring[next]]);
      edges.push([ring[j], nextRing[j]]);
    }
  });
...
const shapeData = buildInfinityLoopData();
