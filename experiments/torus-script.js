...
const buildTorusData = () => {
  ...
  const points = [];
  const edges = [];
  const indexGrid = Array.from({ length: uSegments }, () =>
    Array.from({ length: vSegments }, () => -1)
  );
  ...
  for (let i = 0; i < uSegments; i += 1) {
    ...
    for (let j = 0; j < vSegments; j += 1) {
      ...
      const z = radius * sinU;
      const scale = 1 / (majorRadius + minorRadius);
      indexGrid[i][j] = points.length;
      points.push({
        x: x * scale,
        y: y * scale,
        z: z * scale,
  ...
  for (let i = 0; i < uSegments; i += 1) {
    for (let j = 0; j < vSegments; j += 1) {
      const current = indexGrid[i][j];
      const nextU = indexGrid[(i + 1) % uSegments][j];
      const nextV = indexGrid[i][(j + 1) % vSegments];
...
const shapeData = buildTorusData();
